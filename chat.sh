...
function search() {
  ...
  seq 100 254 | xargs -I {} -P 50 bash -c "echo 'message' | nc -q 0 ${base_ip}{} 9999" >>$LOGFILE 2>$ERRFILE &
  pid=$!
...
function tchat() {
  ...
  if [ "$1" = "-c" ]; then
    ...
    sleep 8
    echo "" | nc -q 0 $ip_connection 9999 >>$LOGFILE 2>$ERRFILE &
 fi
  ...
  if [ "$1" = "-f" ]; then
    ...
    ip_connection=$(cat errchat.log | awk '/Connection stablished/ {print $4}')
    echo -e "Connection stablished w/ $RED$ip_connection$RESET"
  fi
