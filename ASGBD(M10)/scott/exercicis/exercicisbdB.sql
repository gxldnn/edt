...
SELECT pednum, importe, c.nombre "Nombre Cliente", r.nombre"Nombre Representante", o.ciudad"Ciutat de la Oficina" FROM pedido p
JOIN cliente c ON c.cliecod = p.cliecod
JOIN repventa r ON r.repcod = p.repcod
JOIN oficina o ON o.ofinum = r.ofinum

WHERE importe > 150 AND fecha = (SELECT MIN(fecha) FROM pedido WHERE c.cliecod = p.cliecod);
